...
I also have =sleep= for 1s to allow us to read any output.
...
basically) on when on itself, we set =interactive_command= to true here.
So the next time the user enters an command, we run it and exit!
